...
export default function SidebarMenu({ user }) {
  ...
  const signOut = () => {
    ...
    navigate("/gen-chat-web/", { user: null });
  };
  ...
  return (
    <div className="flex">
      <div
        className={` ${
          open ? "w-72" : "w-20 "
        } bg-blue-400 h-screen p-5 relative duration-300`}
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 24 24"
          fill="white"
          className={`absolute cursor-pointer -right-3 top-9 w-6 border-blue-400 border-2 rounded-full ${
            !open && "rotate-180"
          } bg-blue-400`}
        ...
        <ul className="pt-10 h-screen flex-col">
          {svgs.map((svg, index) => (
            <li
              ...
              onClick={() => {
                if (svg.onClick == "signOut") signOut();
              }}
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
